...
class MusicPage extends Component {
        ...
        return (
            <div>
                <List className="title__padding">
                    ...
                    <ListItem>
                        <ListItemContent icon="music_note"><b>Instrumentation: </b>{this.state.musicGet.instrumentation}
                        </ListItemContent>
                    </ListItem>
                </List>
        ...
        this.state.musicResourceGet.map(item => {
            if (item.type === "file" && item.extension === "pdf") {
                musicResourceItems.push(
                    <MusicResourceItem key={item.resource_id} musicResource={item}
                                       musicID={this.props.match.params.musicID} orgID={this.props.match.params.orgID}
                                       history={this.props.history}/>
                )
            } else if (item.type === "youtube_link") {
                youtubeItems.push(
                    <YoutubeItem key={item.resource_id} youtubeItem={item} musicID={this.props.match.params.musicID}
                                 orgID={this.props.match.params.orgID} history={this.props.history}/>
                )
        ...
        return (
            <div>
                ...
                <FABButton style={{margin: '10px', float: "right"}} colored ripple
                           onClick={() => this.props.history.push('/organizations/' + this.props.match.params.orgID + '/musicResource/' + this.props.match.params.musicID)}>
    ...
    renderActiveTabContent() {
        switch (this.state.activeHeaderTab) {
            ...
            case 1:
                return this.renderMusicResources();
            default:
